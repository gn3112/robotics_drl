...
class ReplayMemory():
  def __init__(self, device, history_length, discount, multi_step, priority_weight, priority_exponent, capacity):
    self.device = device
  ...
  def append(self, state, low_state, action, reward, terminal):
    if state.size()[0] > 4:
      state = state[8:,:,:].mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))  # Only store last frame and discretise to save memory
    else:
      state = state.mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))  # Only store last frame and discretise to save memory
    self.transitions.append(Transition(self.t, state, low_state, action, reward, not terminal), self.transitions.max)  # Store new transition with maximum priority
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    low_state = torch.tensor(transition[self.history - 1].low_state, dtype=torch.float32, device=self.device)
    low_next_state = torch.tensor(transition[self.history].low_state, dtype=torch.float32, device=self.device)
    # Discrete action to be used as index
    action = torch.tensor(transition[self.history - 1].action, dtype=torch.float32, device=self.device)
    ...
    return prob, idx, tree_idx, state, low_state, action, R, next_state, low_next_state, nonterminal

  def sample(self, batch_size):
    p_total = self.transitions.total()  # Retrieve sum of all priorities (used to create a normalised probability distribution)
    segment = p_total / batch_size  # Batch size number of segments, based on sum over all probabilities
    batch = [self._get_sample_from_segment(segment, i) for i in range(batch_size)]  # Get batch of valid samples
    probs, idxs, tree_idxs, states, low_states, actions, returns, next_states, low_next_states, nonterminals = zip(*batch)
    states, next_states, = torch.stack(states), torch.stack(next_states)
    low_states, low_next_states = torch.stack(low_states), torch.stack(low_next_states)
    actions, returns, nonterminals = torch.cat(actions), torch.cat(returns), torch.stack(nonterminals)
    probs = np.array(probs, dtype=np.float32) / p_total  # Calculate normalised probabilities
    capacity = self.capacity if self.transitions.full else self.transitions.index
    weights = (capacity * probs) ** -self.priority_weight  # Compute importance-sampling weights w
    weights = torch.tensor(weights / weights.max(), dtype=torch.float32, device=self.device)  # Normalise by max importance-sampling weight from batch
    return tree_idxs, states, low_states, actions, returns, next_states, low_next_states, nonterminals, weights
