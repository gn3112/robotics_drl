...
def train(BATCH_SIZE, DISCOUNT, ENTROPY_WEIGHT, HIDDEN_SIZE, LEARNING_RATE, MAX_STEPS,POLYAK_FACTOR, REPLAY_SIZE, TEST_INTERVAL, UPDATE_INTERVAL, UPDATE_START,logdir):
    ...
    critic_1 = Critic(HIDDEN_SIZE, 1, state_action= True if continuous else False).double().to(device)
    critic_2 = Critic(HIDDEN_SIZE, 1, state_action= True if continuous else False).double().to(device)
