...
class MPCPlanner():
  def __init__(self, action_size, planning_horizon, optimisation_iters, candidates, top_candidates):
    ...
    self.planning_horizon = planning_horizon # ?? 12 in the paper
  ...
  def __call__(self, belief, state, transition_model, reward_model):
    ...
    for i in range(self.optimisation_iters):
      ...
      for t in range(self.planning_horizon):
        next_belief, next_state, _, _ = transition_model(states[-1], actions[:, t], beliefs[-1]) # model free run simulation for candidates?
      ...
      # Update belief with new means and standard deviations  # TODO: Check std dev formula in paper?
      action_distribution = Normal(best_actions.mean(dim=0), best_actions.std(dim=0, unbiased=True))
